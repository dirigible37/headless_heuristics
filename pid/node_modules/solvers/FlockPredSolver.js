define([], function () {
function FlockPredSolver(Problem, Parameters) {

		this.tooClose = Parameters["flockParam1"];
		var personalWeight = Parameters["flockParam2"]; 
		var groupWeight = Parameters["flockParam3"];
		var inertiaWeight = Parameters["flockParam4"];
		var n = Parameters["flockParam5"];
		...
		var predator = birdPos[_.random(1,n-1)].slice();
		var predatorVel = birdVel[_.random(1,n-1)].slice();
		var followBest = 0;
		...
		this.step = function() {
			...
			for(var i = 0; i < n; i++) {
				...
				if(tooClose(predator, birdPos[i]) < this.tooClose){
					...
					birdVel[i] = vecScale(birdVel[i], 2.0);
				}
			...
			var predWeight = (0.39) * Math.random() + 0.6;
...
return {Solver: FlockPredSolver, Params: [           
{"name": "Too Close",
	"id" : "flockParam1",
	"type": "float",
	"value": 0.67},
